...
class UserRepository @Inject constructor(
    ...
    fun observeSignedInUser(account: UserAccount): LiveData<IoResult<UserToken>> = resultLiveData(
        ...
        saveCallResult = { apiToken ->
            Log.d(TAG, "\n${account.googleIdToken.substring(0..31)}\nvs\n${apiToken.googleTokenStub}")
            if (account.googleIdToken.substring(0..31) == apiToken.googleTokenStub) {
                userDao.insertUserToken(apiToken.composeToUserToken(account))
                Log.i(TAG, "Saved user $account")
            }
            else {
                throw Exception("Returned token does not match user: $apiToken !~ $account")
            }
        }
    )
}
